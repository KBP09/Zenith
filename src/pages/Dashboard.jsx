  ...
  useEffect(() => {
    ...
    setTimeout(() => {
      ...
      setData(prevData => ({
        ...
        websiteUsage: fakeData.websiteUsage.map(site => ({ ...site, value: Math.floor(Math.random() * 10000) + 5000 })),
        moodData: historicalData,
        topApps: fakeData.topApps.map(app => ({ ...app, time: Math.floor(Math.random() * 5000) + 1000, change: Math.floor(Math.random() * 20) - 10 })),
      }));
      setIsLoading(false);
    }, 1000);
  }, [timeRange]);
  ...
  return (
    <div className='flex min-h-screen'>
      ...
      <div className="min-h-screen w-full bg-gradient-to-br from-gray-900 to-blue-900 text-white p-8">
        <Header name={data.name} streak={data.streak} />
        <TimeRangeSelector timeRange={timeRange} setTimeRange={setTimeRange} />
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-6">
          <MetricCard title="Focus Score" value={data.focusScore} icon="🎯" color="text-blue-400" change={3} />
          <MetricCard title="Productivity" value={data.productivityScore} icon="📈" color="text-green-400" change={-2} />
          <MetricCard title="Wellbeing" value={data.wellbeingScore} icon="🌿" color="text-yellow-400" change={5} />
          <MetricCard title="Content Consumption" value={data.contentConsumptionScore} icon="📚" color="text-purple-400" change={-1} />
        </div>
          ...
          <ChartCard title="Productivity Trend">
            <ResponsiveContainer width="100%" height={200}>
              <AreaChart data={data.moodData}>
                ...
                <Area type="monotone" dataKey="productivity" stroke="#8884d8" fill="#8884d8" fillOpacity={0.8} />
          ...
          <ChartCard title="Wellbeing Factors">
            <ResponsiveContainer width="100%" height={200}>
              <RadarChart data={data.wellbeingData}>
                ...
                <Radar name="Wellbeing" dataKey="A" stroke="#FF9500" fill="#FF9500" fillOpacity={0.6} />
                <Tooltip contentStyle={{ backgroundColor: '#1a2035', border: 'none' }} />
              </RadarChart>
            </ResponsiveContainer>
          </ChartCard>
          <ChartCard title="Productivity Factors">
            <ResponsiveContainer width="100%" height={200}>
              <RadarChart data={data.productivityData}>
                ...
                <Radar name="Productivity" dataKey="A" stroke="#34C759" fill="#34C759" fillOpacity={0.6} />
                <Tooltip contentStyle={{ backgroundColor: '#1a2035', border: 'none' }} />
              </RadarChart>
            </ResponsiveContainer>
          </ChartCard>
          <ChartCard title="Creativity Factors">
            <ResponsiveContainer width="100%" height={200}>
              <RadarChart data={fakeData.creativity}>
                ...
                <Radar name="Creativity" dataKey="A" stroke="#AF52DE" fill="#AF52DE" fillOpacity={0.6} />
                <Tooltip contentStyle={{ backgroundColor: '#1a2035', border: 'none' }} />
              </RadarChart>
            </ResponsiveContainer>
          </ChartCard>
